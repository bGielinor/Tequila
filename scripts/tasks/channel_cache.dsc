channel_cache:
  ...
  speed: 0.2
  script:
    - foreach <yaml[configurations].read[groups]> key:group_name as:group_id:
      - define channels <discordgroup[bot,<[group_id]>].channels>
      ...
      - if !<yaml[discord_channels].list_keys[<[group_name]>].exclude[<[channels].parse[id]>].is_empty>:
        - foreach <yaml[discord_channels].list_keys[<[group_name]>].exclude[<[channels].parse[id]>]> as:deprecated_channel_id:
          - define deprecated_channel <discordchannel[bot,<[channel_id]>]>
    ...
    - run save_yaml def:discord_channels
